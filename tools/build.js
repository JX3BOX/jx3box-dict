...
const { TABLE_DEFAULT_ROW_MODE, readFile, parseTable, ensureDir, writeFile } = require("@jx3box/jx3box-build-common/file");
...
const readFilters = async () => {
    ...
    const skillKungfuFilePath = path.resolve(__dirname, "../raw/skill_kungfu.txt");
    ...
    const skillOpenLevelFilePath = path.resolve(__dirname, "../raw/skill_open_level.txt");
    const skillOpenLevel = await parseTable(await readFile(skillOpenLevelFilePath), {
    ...
    const tenExtraPointFilePath = path.resolve(__dirname, "../raw/tenextrapoint.tab");
    const tenExtraPoint = await parseTable(await readFile(tenExtraPointFilePath), {
        useDefaultRow: TABLE_DEFAULT_ROW_MODE.NO,
        keepColumns: ["SkillID1", "SkillID2", "SkillID3", "SkillID4", "SkillID5"]
    });
    for (let row of tenExtraPoint) {
        for (let skill of Object.values(row))
            if (skill)
                ret.add(~~skill);
    }
...
const buildSkillsSimp = async (filter) => {
    ...
    logger.info(`简体中文 skill 读取完毕，当前共有 ${Object.keys(ret).length} 个技能`);
    return ret;
};
...
const buildSkillsTrad = async (filter) => {
    ...
    const skillTrad = await kbt.parseKBT(skillTradFilePath,
        [0, 1, 10],
        {
            "0": "SkillID",
            "1": "Level",
    ...
    logger.info(`繁体中文 skill 读取完毕，当前共有 ${Object.keys(ret).length} 个技能`);
    return ret;
};
...
const readBuffsSimp = async () => {
    ...
    logger.info(`简体中文 buff 读取完毕，当前共有 ${Object.keys(ret).length} 个 buff`);
    return ret;
};
...
const readBuffsTrad = async () => {
    ...
    const buffTrad = await kbt.parseKBT(buffTradFilePath,
        [0, 1, 2, 5, 7],
        {
            "0": "BuffID",
            "1": "Level",
    ...
    logger.info(`繁体中文 buff 读取完毕，当前共有 ${Object.keys(ret).length} 个技能`);
    return ret;
};
    ...
    for (let id of Object.keys(simp)) {
        for (let level of Object.keys(simp[id])) {
            ...
            if(!tradName) {
                ...
                logger.warn(`繁体中文数据中无法找到 [${id}-${level}] ${simpName}，使用最高等级 ${tradMaxLevel}`);
                tradName = trad[id][tradMaxLevel];
            }

            // 简繁体相同的不加入
            if (simpName !== tradName)
                mapping[simpName] = tradName;
        }
    }
    return mapping;
};
...
const main = async () => {
    ...
    try {
        ...
        logger.info(`skill 映射处理完毕，共 ${Object.keys(skillsMapping).length} 个技能`);
        const skillOutputPath = path.resolve(__dirname, "../temp/SkillNames.tsv");
        ...
        logger.info(`buff 映射处理完毕，共 ${Object.keys(buffsMapping).length} 个 buff`);
