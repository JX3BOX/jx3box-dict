...
const fs = require("fs");
const { readFile, writeJSON } = require("@jx3box/jx3box-build-common/file");
...
const main = async () => {
    ...
    try {
        ...
        let jx3boxSkillAndBuff = await buildMapping([
            path.resolve(__dirname, "../raw/JX3BOX/SkillNames.tsv"),
            path.resolve(__dirname, "../raw/JX3BOX/BuffNames.tsv")
        ]);

        logger.info("输出技能与 buff 字典");
        const jx3boxSkillAndBuffPath = path.resolve(__dirname, "../output/skill.json");
        await writeJSON(jx3boxSkillAndBuffPath, jx3boxSkillAndBuff);
